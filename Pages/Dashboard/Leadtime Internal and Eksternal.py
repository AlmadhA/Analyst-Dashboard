...
def create_stylish_line_plot(df, x_col, y1_col, y2_col, title="Stylish Line Plot", x_label="X", y_label="Values"):
    ...
    trace1 = go.Scatter(
        ...
        mode='lines+markers',
        name=f'{y1_col}',
        line=dict(color=px.colors.sequential.RdBu[1], width=2),
        marker=dict(size=8)
    )
    ...
    trace2 = go.Scatter(
        ...
        mode='lines+markers',
        name=f'{y2_col}',
        line=dict(color='#ffbca4', width=2),
        marker=dict(size=8)
    )
...
with internal_tab:
    ...
    with kol[0]:
        ...
        st.dataframe(df_resto.set_index('Month').style.format(lambda x: '' if x == 0 else '{:,.0f}'.format(x)).background_gradient(cmap='Reds', axis=0, subset=['CTP']))
    with kol[1]:
        create_stylish_line_plot(df_resto, 'Month', 'CTP', 'Non CTP', '','Total','Month')
